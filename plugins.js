   ...
   if (pairingCode && !Maria.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : +201159355738")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Your WhatsApp bot number\nFor example: +201159355738 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
...
Maria.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
console.log(chalk.green('🟨Welcome to Ultered Maria-md'));
...
"sourceUrl": `${link}`}}})
                	...
                	const Mariabuffer = await getBuffer(ppuser)
                    const Mariatime = moment.tz('Africa/Cairo').format('HH:mm:ss')
	                const Mariadate = moment.tz('Africa/Cairo').format('DD/MM/YYYY')
                	let MariaName = num
